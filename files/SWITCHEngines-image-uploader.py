...
def main():
    ...
    parser.add_argument('-C','--singleDistros', help='DISTROS info file (optional). If not specified, the default /home/ubuntu/distrosInfo is used instead')
    parser.add_argument('-D','--distros', help=' DISTROS (optional) specified within quotes (e.g. \"ubuntu debian\"). If not specified, the distros defined in the DISTROS info file are used')
    parser.add_argument('-Text','--text', help='Text to add as a prefix to the images names (e.g. test)')
    parser.add_argument('-U','--url', help='Url of the location where the raw images are to be found. Default is local directory `/usr/share/nginx/html/SWITCHengines/`')
...
def glanceImageUpdate(id, description_old,name,today):
    ...
    try:
        name=name+"-"+str(today)
        glance_output=glance.images.update(image_id=id,name=name,visibility="private",description=description_old)
    except Exception as e:
        ...
        if not len(existingImagesIdsFinal[i]) > 1:
            ...
            if not   os.path.isfile(url+str(i)+'_ERROR'):
                ...
                if exit_status == 0 and len(existingImagesIdsFinal[i]) == 1:
                    ...
                    if  update_status != 0:
                        fileCreate('/tmp/ERROR_UPDATE_IMAGES_GLANCE_'+ engines_name+'_'+region,"Exit status="+str(update_status) + " " \
